  ...
  grunt.registerTask('yaml', 'Generates app.yaml',
      function() {
        ...
        var logCallback = function(error, result, code) {
          ...
          /**
           * Copy index.yaml
           * This way it will not be overwritten by 'copy' tasks
           * but it will get copied on release
           */
          var indexData = grunt.file.read('index.yaml.base');
          grunt.log.writeln('Generating index yaml');
          grunt.file.write([targetDirectory, 'index.yaml'].join('/'), indexData);
        };
